...
while true; do
  EXERCISE_DIRS=($(find CPP_01 -name "ex*" | sort))
  ...
  if [[ "$choice" =~ ^[0-9]+$ ]]; then
    ...
    if [[ "$opt" == "Quit" ]]; then
      ...
      if [[ -z "$exe" ]]; then
        ...
        if [[ "$opt" == "CPP_01/ex04" ]]; then
          read -p "Enter three parameters (filename and two strings): " arg1 arg2 arg3
          echo -e "${BLUE}Running ${NC}": $exe $arg1 $arg2 $arg3${NC}
          sleep 1
          "$exe" $arg1 $arg2 $arg3
        elif [[ "$opt" == "CPP_01/ex06" ]];then
          read -p "this program takes one parameter: " arg1
          echo -e "${BLUE}Running ${NC}": $exe $arg1${NC}
          sleep 1
          "$exe" $arg1
        else
