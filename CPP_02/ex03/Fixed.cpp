...
Fixed Fixed::operator=(const Fixed &nw) const
{
  std::cout << "Copy assignment operator called" << std::endl;
  Fixed result;
  if(this != &nw)
    result.fixedPoint = nw.fixedPoint;
  return (result);
}
...
Fixed Fixed::operator+(const Fixed &val2) const
{
  Fixed result;
  result.fixedPoint = (fixedPoint + val2.fixedPoint);
  return result;
}
...
{
  Fixed result;
  result.fixedPoint = (fixedPoint * val2.fixedPoint) / (1 << bits);
  return result;
}
...
{
  Fixed result;
  result.fixedPoint = (fixedPoint << bits) / val2.fixedPoint;
  return result;
}
